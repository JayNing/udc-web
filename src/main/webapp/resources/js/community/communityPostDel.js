...
app.controller('communityposdController', function ($scope, $http,httpService,$timeout) {
    ...
    httpService.getList(data,"/bbsManager/queryDisInfoByDisId").then(function successCallback(response) {
        if (response.data.code==1){
            ...
            $scope.content = response.data.result.EssayContent;
    ...
    $scope.changemessage=function(){
        if($scope.message.length>150){
            layer.msg("请输入150个字以内的内容！");
            $scope.message=$scope.message.substr(0,150);
        }
    }
    $scope.submit=function(){
        var data = {
            comContent:$scope.message,
            essayId:$scope.DisRelationId,
            disId:$scope.DisId
        }
        ...
        httpService.postList(data,"/bbsManager/addCommentInfo").then(function successCallback(response) {
            layer.msg(response.data.msg);
            $timeout(function(){
